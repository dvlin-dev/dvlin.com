...
> list_remote_agents 来列出可用的 Agent server，以便让 LLM 去委托任务。
...
接受 task 和上下文，负责状态的处理和流转。相当于 function call 的 tool_call 处理

状态：submitted, working, input-required, completed, failed, canceled

#### Agent card

用来描述 agent 的能力，包含名字、描述、技能等信息，相当于 function call 的 function 定义

被托管在 https://DOMAIN/.well-known/agent.json上，若其中存在敏感信息，为了安全考虑可添加 apikey 进行防范。
...
A2A 的底层逻辑还是 function call 的调用，是在 MCP 的定义之上，并且把如何利用 functions 进行规划任务、协调通信给标准化了。
